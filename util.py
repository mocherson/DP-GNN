...
#         return self
    ...
    def get_node_distr(self):
        node_distr = torch.zeros(self.max_node_tag)
        n_count = Counter(self.node_tags)
        for k, v in n_count.items():
            node_distr[k] = v
        return node_distr
    
    def get_neighbor_distr(self):
        neighbor_distr = torch.zeros((len(self.node_tags), self.max_node_tag))
        for i, nb in enumerate(self.neighbors):
            nb_count = Counter([self.node_tags[n] for n in nb])
            for k, v in nb_count.items():
                neighbor_distr[i,k] = v
        return neighbor_distr


def load_data(dataset, path='/data/datasets/graphdata/', degree_as_tag=False):
    '''
    ...
    with open(path+'%s/%s.txt'%(dataset,dataset), 'r') as f:
...
#         deg_list = list(dict(g.g.degree(range(len(g.g)))).values())
    ...
    return g_list, len(label_dict), len(tagset)
...
def load_train_test(dataset, fold_idx, nfolds=10, path='/data/datasets/graphdata/', val=False):
    graphs, num_classes, num_node_classes = load_data(dataset,path)
    with open(path+'%s/%dfold_idx/train_idx-%d.txt'%(dataset,nfolds, fold_idx), 'r') as f:
    ...
    with open(path+'%s/%dfold_idx/test_idx-%d.txt'%(dataset,nfolds, fold_idx), 'r') as f:
        te_idx = [int(s.strip()) for s in f.readlines()]
        test_graphs = [graphs[i] for i in te_idx]
    return train_graphs, test_graphs, val_graphs, num_classes, num_node_classes
...
def load_data_general(dataset, path='/data/datasets/graphdata/', tag_as_fea=True):
    print('loading data')

    egs = pd.read_csv(path+'%s/%s_A.txt'%(dataset, dataset),header=None)
    gind = pd.read_csv(path+'%s/%s_graph_indicator.txt'%(dataset, dataset),header=None)
    glabel = pd.read_csv(path+'%s/%s_graph_labels.txt'%(dataset, dataset),header=None)[0]
    if os.path.isfile(path+'%s/%s_node_labels.txt'%(dataset, dataset)):    
        ndlabel = pd.read_csv(path+'%s/%s_node_labels.txt'%(dataset, dataset),header=None)[0]
    else:
        print('%s, no node label file, all nodes are seen as the same label'%(dataset))
        ndlabel = np.ones(len(gind))
    ndfea = pd.read_csv(path+'%s/%s_node_attributes.txt'%(dataset, dataset),header=None).values
